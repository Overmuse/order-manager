...
pub(crate) async fn get_allocations(client: &Client) -> Result<Vec<Allocation>> {
...
pub(crate) async fn get_positions_by_owner(client: &Client, owner: Owner) -> Result<Vec<Position>> {
...
pub(crate) async fn save_allocation(client: &Client, allocation: Allocation) -> Result<()> {
    trace!("Saving allocation");
    let (owner, sub_owner) = match allocation.owner {
        Owner::House => ("house".to_string(), None),
        Owner::Strategy(owner, sub_owner) => (owner, sub_owner),
    };
    client.execute("INSERT INTO allocations (owner, sub_owner, claim_id, lot_id, ticker, shares, basis) VALUES ($1, $2, $3, $4, $5, $6, $7);", &[
            &owner,
            &sub_owner,
            &allocation.claim_id,
            &allocation.lot_id,
